...
Объединённый модуль, сочетающий первоначальную реализацию CMV₁ и
расширенную версию CMV1_v2. Поддерживает глобальный контекст Ψ-поля,
фрактальные вычисления, расширенные сервисы второго уровня и метрики
устойчивости, фиксируемые на каждом шаге без внешних библиотек.
"""
...
class CMV_1:
    ...
    def __init__(self):
        ...
        # Истории устойчивости и профили
        self.psi_history = []
        self.rho_history = []
        self.coherence_history = []
        self.reverberation_profile = []
    ...
    def Psi(self, t, log=True):
        ...
        previous_density = self._get_previous_density()
        ...
        numerator = (
            ...
            + self.density_coupling * previous_density
            + 0.15 * resonant
        )
    ...
    def SPS(self, fn_ref, rho_reference):
        ...
        if self.rho_history:
            baseline = sum(self.rho_history) / len(self.rho_history)
        else:
            baseline = rho_reference
        if baseline <= 0:
            baseline = 1.0
    ...
    def handshake(self, cmv2=None, cmv3=None):
        ...
        if cmv2 is not None:
            self.link_metrics["handshake_cmv2"] = 1
            if hasattr(cmv2, "connect"):
                cmv2.connect(cmv1=self, cmv3=cmv3)
        if cmv3 is not None:
            self.link_metrics["handshake_cmv3"] = 1
            if hasattr(cmv3, "connect"):
                cmv3.connect(cmv1=self, cmv2=cmv2)
        self.MRP.log(
            "HANDSHAKE",
    ...
    def receive_feedback(self, value):
        ...
        self.link_metrics["feedback_received"] += 1
    ...
    def emit_to_links(self, psi_value, rho_value, coherence_value):
        ...
        if self.ctx.link_cmv2 is not None:
            ...
            self.link_metrics["psi_packets"] += 1
        ...
        if self.ctx.link_cmv3 is not None:
            ...
            if hasattr(target, "ingest"):
                ...
                self.link_metrics["cmv3_ingest"] += 1
    ...
    def _store_frame(self, t, psi, rho, gamma, plan, confidence, feedback):
        ...
        self.link_metrics["frames_emitted"] += 1
        return frame
    ...
    def _emit_frame(self, frame):
        ...
        if self.ctx.link_cmv2 is not None and hasattr(self.ctx.link_cmv2, "receive_frame"):
            self.link_metrics["frames_to_cmv2"] += 1
        if self.ctx.link_cmv3 is not None and hasattr(self.ctx.link_cmv3, "receive_frame"):
            self.link_metrics["frames_to_cmv3"] += 1
    ...
    def _get_previous_density(self):
        if self.rho_history:
    ...
    # ===============================================
    def run(self, steps=10, env_sequence=None, cmv2=None, cmv3=None):
        if cmv2 is not None or cmv3 is not None:
            self.handshake(cmv2=cmv2, cmv3=cmv3)
        self._sync_time()
        ...
        for idx in range(steps):
            ...
            self.rho_history.append(rho_value)
            self.coherence_history.append(coherence_value)
            if len(self.psi_history) > 256:
                self.psi_history = self.psi_history[-128:]
            if len(self.rho_history) > 256:
                self.rho_history = self.rho_history[-128:]
            if len(self.coherence_history) > 256:
                self.coherence_history = self.coherence_history[-128:]
            self.MRP.log(
                "Ψ-state",
            ...
            if len(self.dt_history) > 256:
                self.dt_history = self.dt_history[-128:]
            self._advance_time(psi_value)
            self.link_metrics["steps"] += 1
    ...
    def _run_diagnostics(self):
        ...
        if len(self.psi_history) >= 2:
            delta_psi = self._abs(self.psi_history[-1] - self.psi_history[-2])
        ...
        reverb_samples = self.reverberation_profile[-16:]
        reverb_flips = 0
        if len(reverb_samples) >= 2:
            for prev, curr in zip(reverb_samples[:-1], reverb_samples[1:]):
                if prev[1] == 0:
                    continue
                if prev[1] * curr[1] < 0:
                    reverb_flips += 1
        flip_ratio = 0.0
        if len(reverb_samples) > 1:
            flip_ratio = reverb_flips / (len(reverb_samples) - 1)

        dt_avg = 0.0
        if self.dt_history:
            dt_avg = sum(self.dt_history) / len(self.dt_history)

        diagnostics = {
            "delta_psi": delta_psi,
            "mean_coherence": mean_coherence,
            "mean_rho": mean_rho,
            "reverb_flip_ratio": flip_ratio,
            "log_size": len(self.MRP.records),
            "dt_avg": dt_avg,
            "links": {
                "cmv2": self.ctx.link_cmv2 is not None,
                "cmv3": self.ctx.link_cmv3 is not None,
            },
        }

        self.last_diagnostics = diagnostics
        self.link_metrics["diagnostics_runs"] += 1

        message = (
            f"ΔΨ={delta_psi:.6f}, Γ̄={mean_coherence:.6f}, ρ̄={mean_rho:.6f}, "
            f"flip_ratio={flip_ratio:.3f}, dt̄={dt_avg:.4f}, logs={len(self.MRP.records)}"
        )
        self.MRP.log("DIAGNOSTICS", message, metadata=diagnostics)
        self._perform_post_checks(delta_psi, mean_coherence, mean_rho, flip_ratio, dt_avg)

    def summary(self):
        """Возвращает список структурированных Ψ-фреймов."""
        return [frame.as_dict() for frame in self.frames]

    def get_link_metrics(self):
        """Возвращает счетчики и проценты активности каналов связи."""
        metrics = dict(self.link_metrics)
        steps = metrics.get("steps", 0)

        def ratio(key):
            if steps == 0 or metrics.get(key, 0) == 0:
                return 0.0
            return (metrics[key] / steps) * 100.0

        percentages = {
            "psi_packets": ratio("psi_packets"),
            "cmv3_ingest": ratio("cmv3_ingest"),
            "frames_to_cmv2": ratio("frames_to_cmv2"),
            "frames_to_cmv3": ratio("frames_to_cmv3"),
            "feedback_received": ratio("feedback_received"),
        }

        stability = self.last_diagnostics or {}
        if stability.get("mean_coherence") is not None:
            stability = stability.copy()
            stability["mean_coherence_percent"] = stability["mean_coherence"] * 100.0
            if "mean_rho" in stability:
                stability["mean_rho_percent"] = min(100.0, stability["mean_rho"] * 20.0)
    ...
    def _reverberation(self, t):
        ...
        limited = self._limit_amplitude(echo, 1.5)
        self.reverberation_profile.append((t, limited))
        if len(self.reverberation_profile) > 256:
            self.reverberation_profile = self.reverberation_profile[-128:]
        return limited
    ...
    def _perform_post_checks(self, delta_psi, mean_coherence, mean_rho, flip_ratio, dt_avg):
        """Дополнительные инженерные проверки после цикла."""
        convergence_ok = delta_psi <= 0.05
        coherence_ok = mean_coherence >= 0.8
        density_ok = 0.0 <= mean_rho <= 5.0
        timestep_ok = dt_avg <= self.base_dt

        stability_checks = {
            "psi_convergence": convergence_ok,
            "coherence_threshold": coherence_ok,
            "density_normalized": density_ok,
            "adaptive_timestep": timestep_ok,
        }

        if all(stability_checks.values()):
            self.link_metrics["stability_passes"] += 1

        self.MRP.log(
            "CHECK-STABILITY",
            " | ".join(
                f"{key}={'OK' if value else 'FAIL'}" for key, value in stability_checks.items()
            ),
            metadata=stability_checks,
        )

        reverberation_ok = flip_ratio >= 0.2
        log_ok = len(self.MRP.records) <= self.MRP.max_records
        frames_ok = len(self.frames) > 0 and len(self.monitor_frames) >= len(self.frames)
        links_ok = (self.ctx.link_cmv2 is not None) or (self.ctx.link_cmv3 is not None)

        integration_checks = {
            "reverberation_balance": reverberation_ok,
            "log_rotation": log_ok,
            "frame_capture": frames_ok,
            "links_active": links_ok,
        }

        if all(integration_checks.values()):
            self.link_metrics["integration_passes"] += 1

        self.MRP.log(
            "CHECK-INTEGRATION",
            " | ".join(
                f"{key}={'OK' if value else 'FAIL'}" for key, value in integration_checks.items()
            ),
            metadata=integration_checks,
        )
